...
@beartype
def train_step(params: dict[str, jnp.ndarray], data: jnp.ndarray, target: jnp.ndarray) -> float:
...
# Usage
@jit
def update_state(state: ModelState, grad: dict[str, jnp.ndarray], lr: float) -> ModelState:
    """Update state immutably."""
    new_params = {k: v - lr * grad[k] for k, v in state.params.items()}
    return attrs.evolve(
        state,
        params=new_params,
        step=state.step + 1,
...
def load_config(config_path: str) -> Config:
    ...
    with path.open(encoding="utf-8") as config_file:
        external = ExternalConfig.model_validate_json(config_file.read())
...
def train_once(state: ModelState, _config: Config) -> ModelState:
