...
## Unattended Batch Protocol (Global Autonomy)
When a session is marked for "Unattended Execution":
1. **Implicit Approval:** If all automated quality gates (pytest, ruff, pyright) pass, the "ask_user" tool is bypassed and the phase/track is considered approved.
2. **Auto-Remediation:** All review findings categorized as "High", "Medium", or "Low" are applied automatically via the `replace` or `write_file` tools.
3. **Failure Stop:** If a tool call fails or a test suite cannot be recovered automatically after 3 attempts, the system will HALT and await manual intervention.
4. **Continuous Progression:** Upon track completion, the system must immediately identify and start the next incomplete track in the registry.
