...
def generate_policy_comparison_figure(
    run_dir: Path, output_dir: Path, dpi: int, formats: list
) -> None:
    """Generate F2: Policy comparison forest plot."""
    print("Generating policy comparison forest plot...")
    ...
    ax.errorbar(
        welfare,
        y_pos,
        xerr=[np.subtract(welfare, welfare_ci_lower), np.subtract(welfare_ci_upper, welfare)],
        fmt="o",
        capsize=5,
...
def generate_voi_results_figure(run_dir: Path, output_dir: Path, dpi: int, formats: list) -> None:
    ...
    print("Generating VOI results bar chart...")
...
def generate_evidence_quality_heatmap(
    run_dir: Path, output_dir: Path, dpi: int, formats: list
) -> None:
    """Generate F6: Evidence quality heatmap."""
    print("Generating evidence quality heatmap...")
    ...
    for i in range(len(modules)):
        for j in range(len(quality)):
            text = ax.text(
                ...
                ha="center",
                va="center",
                color="black" if data[i, j] < 2 else "white",
                fontsize=12,
            )
...
def main():
    ...
    parser.add_argument(
        "--output", type=str, default="outputs/figures/v1.0", help="Output directory for figures"
    )
    parser.add_argument(
