...
def main():
    ...
    parser.add_argument("--joint_draws", required=True, help="Path to joint_draws.npy (dict draws with nested groups)")
    ...
    joint = deterministic_subsample(load_draws_npy(Path(args.joint_draws)), int(args.n_draws))
    n = len(joint)

    # Defaults if a group missing in a draw
    defaults = {
        "behavior": dict(baseline_logit=-2.0, policy_shock=1.0, trend=0.01),
        "clinical": dict(baseline_event_rate=0.02, uptake_to_prevention=0.5, prevention_effect=0.6,
                         cost_per_event=20000.0, qaly_loss_per_event=0.3),
        "insurance": dict(base_premium=1000.0, loss_cost=700.0, expense_load=0.2, markup=0.1,
                          adverse_selection_sensitivity=0.3, price_elasticity=1.2),
        "passthrough": dict(base_pass_through=0.7, concentration_slope=-0.3, noise_sd=0.05),
        "data_quality": dict(base_participation_logit=0.0, fear_sensitivity=2.0, base_auc=0.75, auc_sensitivity=0.08, noise_sd=0.01),
    }
    ...
    theta_store: dict[str, list[dict[str, Any]]] = {g: [] for g in ["mapping","behavior","clinical","insurance","passthrough","data_quality"]}
    ...
    for i in range(n):
        ...
        for g in theta_store.keys():
            if g in d:
                theta_store[g].append(d[g])
    ...
    if len(theta_store["mapping"]) == n:
        np.save(out_dir / "theta_mapping.npy", _theta(theta_store["mapping"], ["intercept","beta_allow","beta_caps","beta_enforcement"], n))
    if len(theta_store["behavior"]) == n:
        np.save(out_dir / "theta_behavior.npy", _theta(theta_store["behavior"], ["baseline_logit","policy_shock","trend"], n))
    if len(theta_store["clinical"]) == n:
        np.save(out_dir / "theta_clinical.npy", _theta(theta_store["clinical"], ["baseline_event_rate","uptake_to_prevention","prevention_effect","cost_per_event","qaly_loss_per_event"], n))
    if len(theta_store["insurance"]) == n:
        np.save(out_dir / "theta_insurance.npy", _theta(theta_store["insurance"], ["base_premium","loss_cost","expense_load","markup","adverse_selection_sensitivity","price_elasticity"], n))
    if len(theta_store["passthrough"]) == n:
        np.save(out_dir / "theta_passthrough.npy", _theta(theta_store["passthrough"], ["base_pass_through","concentration_slope","noise_sd"], n))
    if len(theta_store["data_quality"]) == n:
        np.save(out_dir / "theta_data_quality.npy", _theta(theta_store["data_quality"], ["base_participation_logit","fear_sensitivity","base_auc","auc_sensitivity","noise_sd"], n))
