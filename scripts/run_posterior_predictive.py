...
def _build_model_parameters(draws: dict[str, np.ndarray], draw_idx: int) -> ModelParameters:
    ...
    payload = {name: float(values[draw_idx]) for name, values in draws.items()}
    payload["jurisdiction"] = "australia"
...
def simulate_model_outputs(draws: dict[str, np.ndarray], n_draws: int) -> dict[str, np.ndarray]:
    ...
    Posterior predictive outputs are now generated by evaluating the same
    model used by the dashboard and reporting pipeline. Premium divergence is
    measured as the relative premium change between the status quo and the
    moratorium surface, which is the closest active-path analogue to the
    empirical pricing target.
    """
