...
PRIORS = {
    ...
    'adverse_selection_elasticity': {'dist': 'normal', 'mean': 0.08, 'sd': 0.04, 'trunc': (0, None)},
    'demand_elasticity_high_risk': {'dist': 'normal', 'mean': -0.22, 'sd': 0.08, 'trunc': (None, 0)},
    ...
    'research_participation_elasticity': {'dist': 'normal', 'mean': -0.10, 'sd': 0.03, 'trunc': (None, 0)},
...
EMPIRICAL_TARGETS = {
    'testing_uptake': {'value': 0.52, 'ci_lower': 0.48, 'ci_upper': 0.56, 'source': 'Ettema et al. 2021'},
    'premium_divergence': {'value': 0.08, 'ci_lower': 0.03, 'ci_upper': 0.15, 'source': 'Hersch & Viscusi 2019'},
    'deterrence_rate': {'value': 0.18, 'ci_lower': 0.11, 'ci_upper': 0.25, 'source': 'McGuire et al. 2019'},
}
...
def compare_to_targets(simulations: dict, targets: dict) -> dict:
    ...
    for outcome, target in targets.items():
        ...
        # Calculate coverage
        within_ci = np.sum((sim_values >= target['ci_lower']) & 
                          (sim_values <= target['ci_upper'])) / len(sim_values)
...
def generate_plots_data(simulations: dict, targets: dict, output_dir: Path):
    ...
    for outcome in simulations.keys():
        plot_data[outcome] = {
...
def generate_report(comparisons: dict, n_draws: int, output_dir: Path):
    ...
    for outcome, comp in comparisons.items():
        ...
        lines.append("")
        lines.append(f"**Empirical Target:** {comp['target_value']:.2f} (95% CI: {comp['target_ci'][0]:.2f}-{comp['target_ci'][1]:.2f})")
        lines.append(f"**Source:** {comp['source']}")
        lines.append("")
        lines.append(f"**Simulated:** {comp['sim_mean']:.2f} (95% CI: {comp['sim_ci'][0]:.2f}-{comp['sim_ci'][1]:.2f})")
        lines.append("")
    ...
    if overall_pass:
        lines.append("✅ **All outcomes well-calibrated.** Priors are consistent with empirical evidence.")
    else:
        lines.append("⚠️ **Some outcomes poorly calibrated.** Consider prior adjustments:")
        lines.append("")
        for outcome, comp in comparisons.items():
            if comp['coverage'] < 0.5 or abs(comp['bias']) > 0.2:
                lines.append(f"- **{outcome}**: Bias {comp['bias']:+.1%}, Coverage {comp['coverage']:.1%}")
...
def main():
    ...
    parser.add_argument('--n_draws', type=int, default=2000,
                       help='Number of prior draws (default: 2000)')
    parser.add_argument('--seed', type=int, default=20260303,
                       help='Random seed (default: 20260303)')
    parser.add_argument('--output', default='outputs/ppc',
    ...
    for prior_name in PRIORS.keys():
        ...
        print(f"  {prior_name}: mean={np.mean(prior_draws[prior_name]):.3f}, sd={np.std(prior_draws[prior_name]):.3f}")
    ...
    for outcome, values in simulations.items():
        print(f"  {outcome}: mean={np.mean(values):.3f}, 95% CI=[{np.percentile(values, 2.5):.3f}, {np.percentile(values, 97.5):.3f}]")
    ...
    for outcome, comp in comparisons.items():
        coverage_status = "✅" if comp['coverage'] > 0.5 else "⚠️"
        bias_status = "✅" if abs(comp['bias']) < 0.2 else "⚠️"
        print(f"  {outcome}: Coverage={comp['coverage']:.1%} {coverage_status}, Bias={comp['bias']:+.1%} {bias_status}")
