    ...
    for module_key, params in scenario_params.items():
        if module_key == 'module_a':
            if 'baseline_testing_uptake' in params:
                config['module_a_calibration']['parameters']['baseline_testing_uptake']['prior']['mean'] = params['baseline_testing_uptake']
            if 'deterrence_elasticity' in params:
                config['module_a_calibration']['parameters']['deterrence_elasticity']['prior']['mean'] = params['deterrence_elasticity']
            if 'moratorium_effect' in params:
                config['module_a_calibration']['parameters']['moratorium_effect']['prior']['mean'] = params['moratorium_effect']
        
        elif module_key == 'module_c':
            if 'adverse_selection_elasticity' in params:
                config['module_c_calibration']['parameters']['adverse_selection_elasticity']['prior']['mean'] = params['adverse_selection_elasticity']
            if 'demand_elasticity_high_risk' in params:
                config['module_c_calibration']['parameters']['demand_elasticity_high_risk']['prior']['mean'] = params['demand_elasticity_high_risk']
            if 'baseline_loading' in params:
                config['module_c_calibration']['parameters']['baseline_loading']['prior']['mean'] = params['baseline_loading']
        
        elif module_key == 'module_d':
            if 'family_history_sensitivity' in params:
                config['module_d_calibration']['parameters']['family_history_sensitivity']['prior']['mean'] = params['family_history_sensitivity']
            if 'proxy_substitution_rate' in params:
                config['module_d_calibration']['parameters']['proxy_substitution_rate']['prior']['mean'] = params['proxy_substitution_rate']
        
        elif module_key == 'enforcement':
            if 'enforcement_effectiveness' in params:
                config['enforcement_calibration']['parameters']['enforcement_effectiveness']['prior']['mean'] = params['enforcement_effectiveness']
            if 'complaint_rate' in params:
                config['enforcement_calibration']['parameters']['complaint_rate']['prior']['mean'] = params['complaint_rate']
...
def validate_outputs(results: dict, scenario_name: str) -> list:
    ...
    if 'A_100pct' in scenario_name:
        if results.get('testing_uptake', 0) < 0.99:
            issues.append(f"100% uptake scenario should have ~100% testing, got {results.get('testing_uptake')}")
    
    if 'B_0pct_adverse' in scenario_name:
        if results.get('premium_divergence', 0) > 0.01:
            issues.append(f"0% AS scenario should have minimal premium divergence, got {results.get('premium_divergence')}")
    
    return issues


def run_stress_test(scenario_name: str, scenario_config: dict, jurisdiction: str = 'australia') -> dict:
...
def generate_summary(all_results: dict) -> str:
    ...
    for name, results in all_results.items():
        ...
        lines.append(f"- **Enforcement Compliance:** {results.get('enforcement_compliance', 0):.1%}")
    ...
    if passed == total:
        ...
        lines.append(f"⚠️ {passed}/{total} stress tests passed. {total - passed} scenarios had validation issues.")
...
def main():
    parser = argparse.ArgumentParser(description='Run stress tests for genetic discrimination policy model')
    parser.add_argument('--jurisdiction', default='australia', choices=['australia', 'new_zealand'],
                       help='Jurisdiction to test (default: australia)')
    parser.add_argument('--output', default='outputs/stress_tests',
