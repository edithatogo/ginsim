...
def create_module_a_diagram(
    ...
    # Title
    ax.set_title("Module A: Behavior / Deterrence Game", fontsize=14, fontweight="bold", pad=20)
    ...
    # Outcome: Test
    test_rect = Rectangle(
        (outcome_test_x - 0.7, outcome_test_y - 0.5),
        1.4,
        1.0,
        fill=True,
    ...
    # Outcome: No Test
    no_test_rect = Rectangle(
        (outcome_no_test_x - 0.7, outcome_no_test_y - 0.5),
        1.4,
        1.0,
        fill=True,
...
def create_module_c_diagram(
    ...
    high_risk_rect = Rectangle(
        (high_risk_x - 1.5, high_risk_y - 1.5),
        3,
        3,
        fill=True,
        color=config.highlight_color,
    ...
    low_risk_rect = Rectangle(
        (low_risk_x - 1.5, low_risk_y - 1.5),
        3,
        3,
        fill=True,
        color=config.success_color,
...
def create_module_d_diagram(
    ...
    genetic_rect = Rectangle(
        (genetic_x - 1, genetic_y - 0.7),
        2,
        1.4,
        fill=True,
        color=config.highlight_color,
    ...
    insurer_rect = Rectangle(
        (insurer_x - 1, insurer_y - 0.7),
        2,
        1.4,
        fill=True,
        color=config.node_color,
...
def create_module_e_diagram(
    ...
    ax.set_title("Module E: Pass-Through Mechanism", fontsize=14, fontweight="bold", pad=20)
    ...
    if params and "pass_through_rate" in params:
        ...
        for i in range(len(gauge_x) - 1):
            ax.axvspan(
                2 + gauge_x[i] * 8,
                2 + gauge_x[i + 1] * 8,
                ymin=pass_through_y - 0.1,
                ymax=pass_through_y + 0.1,
                color=colors[i],
                alpha=0.5,
            )
    ...
    # Labels
    ax.text(2, pass_through_y - 0.4, "0%", ha="center", va="top", fontsize=config.font_size - 1)
    ax.text(10, pass_through_y - 0.4, "100%", ha="center", va="top", fontsize=config.font_size - 1)
...
def create_module_f_diagram(
    ...
    ax.set_title("Module F: Data Quality Externality", fontsize=14, fontweight="bold", pad=20)

    # Create data quality feedback loop diagram

    # Policy at top
    policy_x, policy_y = 6, 7
    policy_rect = Rectangle(
        (policy_x - 1.5, policy_y - 0.7),
        3,
        1.4,
        fill=True,
        color=config.highlight_color,
    ...
    participation_rect = Rectangle(
        (participation_x - 1.2, participation_y - 0.7),
        2.4,
        1.4,
        fill=True,
        color=config.node_color,
...
def create_enforcement_diagram(
    ...
    ax.set_title("Enforcement: Compliance Game", fontsize=14, fontweight="bold", pad=20)

    # Create compliance game diagram

    # Regulator
    regulator_x, regulator_y = 2, 5
    regulator_circle = Circle(
        (regulator_x, regulator_y),
        0.7,
        fill=True,
        color=config.node_color,
        ec=config.node_edge_color,
    ...
    enforcement_rect = Rectangle(
        (enforcement_x - 1.2, enforcement_y - 1),
        2.4,
        2,
        fill=True,
        color=config.warning_color,
    ...
    comply_rect = Rectangle(
        (comply_x - 1, comply_y - 0.5),
        2,
        1,
        fill=True,
        color=config.success_color,
    ...
    violate_rect = Rectangle(
        (violate_x - 1, violate_y - 0.5),
        2,
        1,
        fill=True,
        color=config.highlight_color,
...
def export_diagram(
    ...
    for fmt in formats:
        ...
        fig.savefig(output_path, dpi=150 if fmt == "png" else None, bbox_inches="tight")
