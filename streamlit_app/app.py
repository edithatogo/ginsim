...
def get_policy_color(policy_name: str) -> str:
    ...
    if 'ban' in name: return STYLE['colors']['statutory_ban']
...
with tab1:
    ...
    if 'result' in st.session_state:
        ...
        with col_a:
            ...
            st.write(f"**Premium High Risk:** {float(res.insurance_premiums['premium_high_risk']):.3f}")
            st.write(f"**Premium Low Risk:** {float(res.insurance_premiums['premium_low_risk']):.3f}")
        ...
        with col_b:
            ...
            st.write(f"**Testing Benefit (QALYs):** {float(res.all_metrics['welfare']['testing_benefit']):.2f}")
            st.write(f"**Consumer Surplus:** {float(res.all_metrics['welfare']['consumer_surplus']):.2f}")
            st.write(f"**Compliance Cost:** {float(res.all_metrics['welfare']['compliance_cost']):.2f}")
    else:
...
with tab3:
    ...
    if 'result' in st.session_state:
        ...
        st.success("For interaction effects and Sobol analysis, visit the [Sensitivity Analysis](/2_Sensitivity) page.")
    else:
...
with col_f1:
    if st.button("🎮 Go to Game Diagrams"): st.switch_page("pages/1_Game_Diagrams.py")
with col_f2:
    if st.button("📊 Go to Sensitivity Analysis"): st.switch_page("pages/2_Sensitivity.py")
with col_f3:
    if st.button("🎯 Go to Scenario Analysis"): st.switch_page("pages/3_Scenarios.py")
