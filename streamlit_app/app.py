...
with tab_main:
    ...
    if "main_result" in st.session_state:
        ...
        w_impact = (
            float(res.equity_weighted_welfare) if use_equity_weights else float(res.welfare_impact)
        )
        w_label = (
            "Net Social Benefit (Equity-Weighted)"
            if use_equity_weights
            else "Net Social Benefit (Utilitarian)"
        )
        ...
        with col_l:
            ...
            fig.update_layout(
                ...
                title=f"Stakeholder Impact (Equity Factor: {e_factor:.2f}x)",
            )
        ...
        with col_r:
            ...
            st.write(
                f"**Information Gap:** {res.all_metrics['proxy']['residual_information_gap']:.1%}"
            )
...
with tab_evidence:
    ...
    st.caption(
        "Equity Localization Engine v1.0 • Māori Health Sovereignty & Vertical Equity Active"
    )
