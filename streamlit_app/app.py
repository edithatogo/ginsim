...
with st.sidebar.expander("⚙️ Advanced Numerical Controls"):
    ...
    # FIXED: match ModelParameters name
    baseline_testing_uptake = st.slider("Baseline Testing Share", 0.1, 0.9, 0.52)
...
if st.sidebar.button("🔬 Run Model", type="primary"):
    with st.spinner("Running core model..."):
    ...
    **Key Takeaway:** Under the **{st.session_state['policy_label']}** regime, we project a testing uptake of **{float(res.testing_uptake):.1%}**. 
    ...
    with tab1:
        ...
        with st.expander("🔍 View Detailed Market Metrics"):
            ...
            # Use fixed keys from evaluate_single_policy
    ...
    with tab2:
        ...
        for pid in ["status_quo", "moratorium", "ban"]:
            r = evaluate_cached(current_params, pid)
