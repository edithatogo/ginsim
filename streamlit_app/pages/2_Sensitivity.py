    ...
    if st.button("🎲 Run PSA Simulation", type="primary"):
        with st.spinner(f"Executing {n_draws} JAX-vectorized simulations..."):
            ...
            with c1:
                ...
                st.caption(f"95% CrI: [{res_psa['uptake'].lower_95:.1%}, {res_psa['uptake'].upper_95:.1%}]")

                # Histogram
                fig_uptake = px.histogram(
                    res_psa["uptake"].samples.tolist(),
                    nbins=30, title="Uptake Distribution",
            ...
            with c2:
                ...
                st.caption(f"95% CrI: [${res_psa['welfare'].lower_95:,.0f}, ${res_psa['welfare'].upper_95:,.0f}]")

                # Histogram
                fig_welfare = px.histogram(
                    res_psa["welfare"].samples.tolist(),
                    nbins=30, title="Welfare Distribution (PPP)",
...
with tab_voi:
    ...
    st.markdown("How much is it worth to resolve all parameter uncertainty before choosing a policy?")
    ...
    if st.button("💎 Calculate VOI Metrics", type="primary"):
        with st.spinner("Computing global welfare matrix..."):
            ...
            st.info("EVPI represents the maximum a society should pay for perfect scientific evidence to eliminate all decision risk.")
            ...
            fig_voi = px.bar(
                evppi_df, x="Value of Information ($)", y="Parameter",
                orientation="h", title="Research Priorities: Which parameter matters most?"
            )
...
with tab_tornado:
    ...
    if st.button("🌪️ Generate Tornado Chart", type="primary"):
        ...
        fig_t.update_layout(title="Impact on Testing Uptake (±25% variation)", template="plotly_white")
