...
if st.sidebar.button("🔍 Run Comparative Analysis", type="primary"):
    with st.spinner("Executing full pipeline for all scenarios..."):
        ...
        for name, config in scenarios.items():
            ...
            if "ban" in name: p_obj = policies.get("ban")
            if "moratorium" in name: p_obj = policies.get("moratorium")
        ...
        # Chart
        fig = go.Figure()
        fig.add_trace(go.Bar(x=df["Scenario"], y=df["Welfare"], marker_color="#0072B2", name="Net Social Benefit"))
        fig.update_layout(title="Societal Welfare by Scenario (DCBA Integrated)", template="plotly_white")
