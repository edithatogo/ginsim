...
from src.model.parameters import PolicyConfig, ModelParameters, get_default_parameters
...
def test_welfare_monotonicity_pbt(params):
    # Standard policies
    sq = PolicyConfig(name="sq", description="SQ", allow_genetic_test_results=True, allow_family_history=True)
    ...
    ban = PolicyConfig(name="ban", description="Ban", allow_genetic_test_results=False, allow_family_history=False)
    ...
    assert res_ban.all_metrics["welfare"]["health_benefits"] >= res_mor.all_metrics["welfare"]["health_benefits"] - 1e-6
    assert res_mor.all_metrics["welfare"]["health_benefits"] >= res_sq.all_metrics["welfare"]["health_benefits"] - 1e-6
